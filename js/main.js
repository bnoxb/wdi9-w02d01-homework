...
// function for just the bond titles
  const bondTitles = [];
...
// Cumulative Gross Bonds
let grossBonds = 0;
for (let i = 0; i<bondFilms.length; i++ ) {
    const thisBondFilm = bondFilms[i];
// class RegExp1 extends RegExp {
//     [Symbol.replace](thisBondFilm.gross) {
//         return RegExp.prototype[Symbol.replace]
//     }
// }
    //const re = new RegExp(/\W+/g);
    //This is the one I figured out using stack overflow and mdn
    //const re = new RegExp(/\$|,+/, 'g');
    //Bryant from LA used this, which is better IMO because its a little easier to read the two brackets than the |
    const re = new RegExp(/[$,]/, 'g');
    const num = parseInt(thisBondFilm.gross.replace(re, ""));
    grossBonds += num;
}
console.log(`James Bond has earned someone $${grossBonds} dollars`);
